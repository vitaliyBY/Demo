    ...
    public class OrganizationAnalyzer : Pullenti.Ner.Analyzer
    {
        static Pullenti.Ner.ReferentToken TryAttachPoliticParty(Pullenti.Ner.Token t, bool onlyAbbrs = false)
        {
            ...
            if ((((nameTok = m_PoliticNames.TryParse(t, Pullenti.Ner.Core.TerminParseAttr.No)))) != null && !t.Chars.IsAllLower) 
            {
                ...
                if (nameTok.BeginToken == nameTok.EndToken && !t.GetMorphClassInDictionary().IsUndefined) 
                    ...
                    coef += 0.5;
                if ((nameTok.EndToken is Pullenti.Ner.TextToken) && (nameTok.EndToken as Pullenti.Ner.TextToken).Term.Contains("ПАРТИ")) 
                    coef += 0.5;
        ...
        static Pullenti.Ner.ReferentToken AttachGlobalOrg(Pullenti.Ner.Token t, AttachType attachTyp, object extGeo = null)
        {
            ...
            if (t.Chars.IsAllUpper && ((term == "СК" || term == "CK"))) 
            {
                ...
                {
                    bool ok1 = false;
                    Pullenti.Ner.Core.AnalyzerData ad = t.Kit.GetAnalyzerDataByAnalyzerName("ORGANIZATION");
                    foreach (Pullenti.Ner.Referent e in ad.Referents) 
                    {
                        foreach (Pullenti.Ner.Slot s in e.Slots) 
                        {
                            ...
                            {
                                if ((s.Value as string).ToUpper().Contains("СЛЕДСТВЕННЫЙ КОМИТЕТ")) 
                                {
                                    ok1 = true;
                                    break;
                                }
            ...
            foreach (Pullenti.Ner.Core.IntOntologyToken ot in gt) 
            {
                ...
                if (((t.Chars.IsAllLower && attachTyp != AttachType.ExtOntology && extGeo == null) && !t.IsValue("МИД", null) && !org0._typesContains("факультет")) && org0.Kind != OrganizationKind.Justice) 
                {
                    ...
                    else if (tyty != null && tyty.EndToken.Next != null && (tyty.EndToken.Next.GetReferent() is Pullenti.Ner.Geo.GeoReferent)) 
                    {
                    }
                    else if (!okk) 
                        continue;
                ...
                if (org == null) 
                {
                    ...
                    Pullenti.Ner.Slot sl = org.FindSlot("NAME", "МИНКУЛЬТ", true);
                    if (sl != null) 
                        org.Slots.Remove(sl);
                }
                if (extGeo != null) 
                ...
                if (t1.WhitespacesAfterCount < 2) 
                {
                    ...
                    if (typ != null) 
                    {
                        if (Pullenti.Ner.Org.Internal.OrgItemTypeToken.IsTypeAccords(org, typ)) 
                        {
                            ...
                            {
                                Pullenti.Ner.ReferentToken rrr = OrganizationAnalyzer.ProcessReferentStat(t1.Next, null);
                                if (rrr != null && !rrr.Referent.CanBeEquals(org, Pullenti.Ner.Core.ReferentsEqualType.WithinOneText)) 
                                {
                                }
                                else 
                                {
                ...
                gt = Pullenti.Ner.Org.Internal.OrgGlobal.GlobalOrgs.TryAttach(t1.Next, null, true);
                if (gt != null && gt[0].Item.Referent == org0 && (t1.WhitespacesAfterCount < 3)) 
                    t1 = gt[0].EndToken;
                else if (Pullenti.Ner.Core.BracketHelper.CanBeStartOfSequence(t1.Next, false, false)) 
                {
                    gt = Pullenti.Ner.Org.Internal.OrgGlobal.GlobalOrgs.TryAttach(t1.Next.Next, null, true);
                    if (gt != null && gt[0].Item.Referent == org0) 
                    {
                        if (Pullenti.Ner.Core.BracketHelper.CanBeEndOfSequence(gt[0].EndToken.Next, false, null, false)) 
        ...
        static Pullenti.Ner.ReferentToken _tryAttachOrgNames(Pullenti.Ner.Token t)
        {
            ...
            if ((tok == null && !ok && tName1 == null) && prof == OrgProfile.Undefined) 
            {
                if (br == null || !t.Chars.IsCapitalUpper) 
                    return null;
                if (!(t00 is Pullenti.Ner.TextToken)) 
                    return null;
            ...
            if ((((prof & OrgProfile.Media)) != OrgProfile.Undefined) && t0.Previous != null) 
            {
                if (t0.Previous.IsValue("ГРУППА", null) || t0.Previous.IsValue("РОКГРУППА", null)) 
                {
                    ...
                    org.AddProfile(OrgProfile.Culture);
                }
        ...
        static void _initSport()
        {
            ...
            foreach (string s in new string[] {"музыкант", "певец", "певица", "ударник", "гитарист", "клавишник", "солист", "солистка", "исполнять", "исполнить", "концерт", "гастроль", "выступление", "известный", "известнейший", "популярный", "популярнейший", "рокгруппа", "панкгруппа", "артгруппа", "группа", "пластинка", "грампластинка", "концертный", "музыка", "запись", "студия"}) 
            {
                m_PropPref.Add(new Pullenti.Ner.Core.Termin(s.ToUpper()) { Tag = OrgProfile.Music });
            }
        }
        ...
        /// </summary>
        public static bool BanAutoAbbreviations = false;
                ...
                if (ofTok == null) 
                {
                    if (org.Higher == null) 
                    {
                        if (!Pullenti.Ner.Org.Internal.OrgOwnershipHelper.CanBeHigher(org1, org, false)) 
                        {
                            ...
                            if ((t1.Previous == t && (t1.WhitespacesBeforeCount < 3) && org.Names.Count == 0) && org.Number == null && org.Profiles.Contains(OrgProfile.Unit)) 
                            {
                                if (Pullenti.Ner.Org.Internal.OrgOwnershipHelper.CanBeHigher(org1, org, false)) 
                                    ok = true;
                            }
        ...
        internal static Pullenti.Ner.MetaToken _checkAliasAfter(Pullenti.Ner.ReferentToken rt, Pullenti.Ner.Token t)
        {
            ...
            while (t != null) 
            {
                if (!(t is Pullenti.Ner.TextToken)) 
                    ...
                    t = t.Next;
                else if ((t.Morph.Class.IsPreposition || t.Morph.Class.IsMisc || t.IsValue("ИМЕНОВАТЬ", null)) || t.IsValue("ТЕКСТ", null) || t.IsValue("ТАКЖЕ", null)) 
                    t = t.Next;
                else 
                    break;
            }
            ...
            if (((step == 0 && rt == null && tt != null) && !tt.Chars.IsAllLower && tt.Chars.IsCyrillicLetter) && tt.GetMorphClassInDictionary().IsUndefined) 
            {
                ...
                if ((((s.StartsWith("ГУ") || s.StartsWith("РУ") || s.StartsWith("МУ")) || s.StartsWith("МО"))) && s.Length > 3 && ((s.Length > 4 || s == "ГУВД"))) 
                {
                    ...
                    if (rt != null) 
                    {
                        ...
                        if (s.StartsWith("МО")) 
                            typ.Root = Pullenti.Ner.Org.Internal.OrgItemTypeToken.m_MejmunOtdel;
            ...
            if (r != null) 
            {
                if (r.TypeName == GEONAME) 
                {
                    if ((t as Pullenti.Ner.ReferentToken).BeginToken.IsValue("ТЕРРИТОРИЯ", null)) 
                        return null;
                    if (t.WhitespacesBeforeCount <= 20 || t.Morph.Case.IsGenitive) 
                        return r;
                }
