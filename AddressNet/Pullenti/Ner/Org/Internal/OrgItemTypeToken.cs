...
namespace Pullenti.Ner.Org.Internal
{
    public class OrgItemTypeToken : Pullenti.Ner.MetaToken
    {
        ...
        public static void Initialize()
        {
            ...
            foreach (string s in new string[] {"ФИРМА", "КОМПАНИЯ", "КОРПОРАЦИЯ", "ГОСКОРПОРАЦИЯ", "КОНЦЕРН", "КОНСОРЦИУМ", "ХОЛДИНГ", "МЕДИАХОЛДИНГ", "ТОРГОВЫЙ ДОМ", "ТОРГОВЫЙ ЦЕНТР", "ТОРГОВО РАЗВЛЕКАТЕЛЬНЫЙ ЦЕНТР", "УЧЕБНЫЙ ЦЕНТР", "ИССЛЕДОВАТЕЛЬСКИЙ ЦЕНТР", "КОСМИЧЕСКИЙ ЦЕНТР", "ДЕЛОВОЙ ЦЕНТР", "БИЗНЕС ЦЕНТР", "БИЗНЕС ПАРК", "АУКЦИОННЫЙ ДОМ", "ИЗДАТЕЛЬСТВО", "ИЗДАТЕЛЬСКИЙ ДОМ", "ТОРГОВЫЙ КОМПЛЕКС", "ТОРГОВО РАЗВЛЕКАТЕЛЬНЫЙ КОМПЛЕКС", "ТОРГОВО ОФИСНЫЙ КОМПЛЕКС", "ТОРГОВО ОФИСНЫЙ ЦЕНТР", "СПОРТИВНЫЙ КОМПЛЕКС", "СПОРТИВНО РАЗВЛЕКАТЕЛЬНЫЙ КОМПЛЕКС", "СПОРТИВНО ОЗДОРОВИТЕЛЬНЫЙ КОМПЛЕКС", "ФИЗКУЛЬТУРНО ОЗДОРОВИТЕЛЬНЫЙ КОМПЛЕКС", "АГЕНТСТВО НЕДВИЖИМОСТИ", "ГРУППА КОМПАНИЙ", "МЕДИАГРУППА", "МАГАЗИН", "ТОРГОВЫЙ КОМПЛЕКС", "ГИПЕРМАРКЕТ", "СУПЕРМАРКЕТ", "КАФЕ", "РЕСТОРАН", "БАР", "ТРАКТИР", "ТАВЕРНА", "СТОЛОВАЯ", "АУКЦИОН", "АНАЛИТИЧЕСКИЙ ЦЕНТР", "COMPANY", "CORPORATION"}) 
            {
            ...
            foreach (string s in new string[] {"ПАНСИОНАТ", "САНАТОРИЙ", "ДОМ ОТДЫХА", "ОТЕЛЬ", "ГОСТИНИЦА", "ГОСТИНИЧНЫЙ КОМПЛЕКС", "SPA-ОТЕЛЬ", "ОЗДОРОВИТЕЛЬНЫЙ ЛАГЕРЬ", "ДЕТСКИЙ ЛАГЕРЬ", "ПИОНЕРСКИЙ ЛАГЕРЬ", "БАЗА ОТДЫХА", "СПОРТ-КЛУБ", "ФИТНЕС-КЛУБ"}) 
            {
                t = new OrgItemTypeTermin(s) { Coeff = 3, Typ = OrgItemTypeTyp.Org, CanHasLatinName = true, CanHasSingleName = true, MustHasCapitalName = true };
                if (s == "САНАТОРИЙ") 
                    t.AddAbridge("САН.");
        ...
        public static OrgItemTypeToken TryAttach(Pullenti.Ner.Token t, bool canBeFirstLetterLower = false)
        {
            ...
            if (SpeedRegime && ad.TRegime) 
            {
                ...
                if (res != null) 
                {
                    for (Pullenti.Ner.Token tt = t; tt != null && tt.BeginChar <= res.EndToken.BeginChar; tt = tt.Next) 
                    {
        ...
        static OrgItemTypeToken TryAttachInt(Pullenti.Ner.Token t, bool canBeFirstLetterLower)
        {
            ...
            if ((res != null && res.Geo == null && (res.WhitespacesBeforeCount < 3)) && (res.BeginToken.Previous is Pullenti.Ner.TextToken) && !res.BeginToken.IsValue("УК", null)) 
            {
            ...
            if (((res != null && res.Geo == null && res.Root != null) && res.Root.Typ == OrgItemTypeTyp.Prefix && (res.EndToken.Next is Pullenti.Ner.ReferentToken)) && (res.EndToken.Next.GetReferent() is Pullenti.Ner.Geo.GeoReferent)) 
            {
                res.EndToken = res.EndToken.Next;
                res.Geo = res.EndToken as Pullenti.Ner.ReferentToken;
            }
            if (res != null || tt == null) 
                return res;
            ...
            else if ((term == "МО" && t.Chars.IsAllUpper && (t.Next is Pullenti.Ner.ReferentToken)) && (t.Next.GetReferent() is Pullenti.Ner.Geo.GeoReferent)) 
            {
                ...
                return new OrgItemTypeToken(t, t) { Typ = "судебный участок", Root = m_SudUch, CanBeOrganization = true };
            ...
            {
                Pullenti.Ner.Org.OrganizationReferent org1 = t.Next.GetReferent() as Pullenti.Ner.Org.OrganizationReferent;
                if (org1 != null || t.Next.IsValue("МВД", null)) 
                    return new OrgItemTypeToken(t, t) { Root = m_MejmunOtdel, Typ = m_MejmunOtdel.CanonicText.ToLower() };
            }
            if (term == "В") 
            {
            ...
            if ((t.Morph.Class.IsAdjective && t.Next != null && (t.WhitespacesAfterCount < 3)) && ((t.Next.Chars.IsAllUpper || t.Next.Chars.IsLastLower))) 
            {
            ...
            if (res == null && t.Morph.Class.IsAdjective) 
            {
                Pullenti.Ner.ReferentToken rt = t.Kit.ProcessReferent("GEO", t, null);
                if (rt != null && rt.Morph.Class.IsAdjective && (rt.WhitespacesAfterCount < 3)) 
                {
                    OrgItemTypeToken next = TryAttach(rt.EndToken.Next, false);
                    if (next != null && next.Geo == null) 
                    {
                        next.BeginToken = t;
                        next.Geo = rt;
                        res = next;
