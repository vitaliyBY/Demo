    ...
    public class OrganizationReferent : Pullenti.Ner.Referent
    {
        ...
        public override string ToStringEx(bool shortVariant, Pullenti.Morph.MorphLang lang = null, int lev = 0)
        {
            ...
            if (!shortVariant && Owner != null) 
                res.AppendFormat("; {0}", Owner.ToStringEx(true, lang, lev + 1));
            if (!shortVariant) 
            {
                if (!noType && !isDep) 
                {
                ...
                if (isDep || outOwnInName) 
                {
                    foreach (Pullenti.Ner.Slot ss in Slots) 
                    {
                        if (ss.TypeName == ATTR_HIGHER && (ss.Value is Pullenti.Ner.Referent) && (lev < 20)) 
                        {
                            ...
                            res.Append(';');
                            res.AppendFormat(" {0}", (ss.Value as Pullenti.Ner.Referent).ToStringEx(shortVariant, lang, lev + 1));
                            break;
                        }
                    }
                }
        ...
        {
            return string.Format("{0} {1}", Kind, this.ToStringEx(true, Pullenti.Morph.MorphLang.Unknown, 0));
        }
        ...
        internal bool CheckCorrection()
        {
            if (Slots.Count < 1) 
                return false;
            string s = this.ToStringEx(true, Pullenti.Morph.MorphLang.Unknown, 0).ToLower();
                ...
                foreach (Pullenti.Ner.Slot s in Slots) 
                {
                    if (s.TypeName == ATTR_GEO && (s.Value is Pullenti.Ner.Geo.GeoReferent)) 
                    {
                        ...
                        if (gg.CanBeEquals(geo, Pullenti.Ner.Core.ReferentsEqualType.WithinOneText) || gg.Higher == geo) 
                            return true;
                        if (this.FindSlot(ATTR_TYPE, "посольство", true) != null || this.FindSlot(ATTR_TYPE, "консульство", true) != null) 
                            break;
        ...
        internal Dictionary<string, bool> NameVars
        {
            get
            {
                ...
                foreach (string n in Names) 
                {
                    ...
                    if (!OrganizationAnalyzer.BanAutoAbbreviations) 
                    {
                        if ((((a = Pullenti.Ner.Core.MiscHelper.GetAbbreviation(n)))) != null && a.Length > 1) 
                        {
                            ...
                            {
                                string aa = string.Format("{0}{1}", a, geo.ToStringEx(true, Pullenti.Morph.MorphLang.Unknown, 0)[0]);
        ...
        bool _checkEqEponyms(OrganizationReferent org)
        {
            ...
            if (this.FindSlot(ATTR_EPONYM, null, true) != null && org.FindSlot(ATTR_EPONYM, null, true) != null) 
                return false;
            string s = org.ToStringEx(true, Pullenti.Morph.MorphLang.Unknown, 0);
            foreach (string e in Eponyms) 
            {
                if (s.Contains(e)) 
                    return true;
            }
            s = this.ToStringEx(true, Pullenti.Morph.MorphLang.Unknown, 0);
        ...
        bool _canBeEquals(Pullenti.Ner.Referent obj, bool ignoreGeoObjects, Pullenti.Ner.Core.ReferentsEqualType typ, int lev)
        {
            ...
            if (this.FindSlot(ATTR_EPONYM, null, true) != null || obj.FindSlot(ATTR_EPONYM, null, true) != null) 
            {
                ...
                else 
                {
                    ...
                    if (this.FindSlot(ATTR_EPONYM, null, true) == null || obj.FindSlot(ATTR_EPONYM, null, true) == null) 
                    {
                        string s = obj.ToStringEx(true, Pullenti.Morph.MorphLang.Unknown, 0);
                        foreach (string e in Eponyms) 
                        {
                            if (s.Contains(e)) 
                            {
