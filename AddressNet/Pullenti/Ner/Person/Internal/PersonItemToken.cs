...
namespace Pullenti.Ner.Person.Internal
{
    public class PersonItemToken : Pullenti.Ner.MetaToken
    {
            ...
            if (tt == null) 
            {
                if (t.Chars.IsLetter && t.Chars.IsCapitalUpper && (t is Pullenti.Ner.ReferentToken)) 
                {
                    ...
                    if (rt.BeginToken == rt.EndToken && !(rt.Referent is Pullenti.Ner.Person.PersonReferent)) 
                    {
                        ...
                        if (res0 == null) 
                        {
                            res0 = new PersonItemToken(rt, rt) { Value = rt.Referent.ToStringEx(true, t.Kit.BaseLanguage, 0).ToUpper(), Chars = rt.Chars, Morph = rt.Morph };
            ...
            foreach (Pullenti.Morph.MorphBaseInfo wff in tt.Morph.Items) 
            {
                ...
                if (wf.Class.IsProperSurname || surPrefix != null) 
                {
                    ...
                    if (adj != null) 
                    {
                        if (!wf.IsInDictionary && adj.Number == Pullenti.Morph.MorphNumber.Singular) 
                        {
                            ...
                            if (val == tt.Term) 
                            {
                                if (!MorphPersonItem.EndsWithStdSurname(wf.NormalCase)) 
                                    break;
                            }
                        }
                        adj = null;
                    ...
                    else if ((v.Gender == Pullenti.Morph.MorphGender.Masculine && !wf.IsInDictionary && ((v.Value.EndsWith("ИН") || v.Value.EndsWith("ІН")))) && (t is Pullenti.Ner.TextToken) && (((t as Pullenti.Ner.TextToken).Term.EndsWith("ИНА") || (t as Pullenti.Ner.TextToken).Term.EndsWith("ІНА")))) 
                    {
                        MorphPersonItemVariant vv = new MorphPersonItemVariant((t as Pullenti.Ner.TextToken).Term, new Pullenti.Morph.MorphBaseInfo() { Case = Pullenti.Morph.MorphCase.Nominative, Gender = Pullenti.Morph.MorphGender.Masculine }, true);
                        vv.Value = (t as Pullenti.Ner.TextToken).Term;
                        vv.Gender = Pullenti.Morph.MorphGender.Masculine;
                        res.Lastname.Vars.Add(vv);
                    }
                    if (wf.IsInDictionary) 
                        res.Lastname.IsInDictionary = true;
                ...
                if (!MorphPersonItem.EndsWithStdSurname(tt.Term)) 
                {
                    if (wf.Class.IsProperSecname) 
                    {
                        ...
                        else if (wf.Misc != null && wf.Misc.Form == Pullenti.Morph.MorphForm.Synonym) 
                            continue;
                ...
                foreach (Pullenti.Morph.MorphBaseInfo wf in tt.Morph.Items) 
                {
                    if (!wf.Class.IsVerb) 
                    {
                        if (wf.ContainsAttr("к.ф.", null)) 
                            continue;
                        if (wf.Case.IsGenitive && wf.Number == Pullenti.Morph.MorphNumber.Plural && tt.Term.EndsWith("ОВ")) 
                        {
                            res.Lastname.Vars.Clear();
                            res.Lastname.Vars.Add(new MorphPersonItemVariant(tt.Term, new Pullenti.Morph.MorphBaseInfo() { Gender = Pullenti.Morph.MorphGender.Masculine, Case = Pullenti.Morph.MorphCase.Nominative }, true));
                            res.Lastname.IsLastnameHasStdTail = true;
                            break;
                        }
        ...
        public static List<PersonItemToken> TryAttachList(Pullenti.Ner.Token t, ParseAttr attrs = ParseAttr.No, int maxCount = 10)
        {
            ...
            if (res.Count == 1 && res[0].LengthChar == 1) 
                return null;
            if (((res.Count >= 4 && res[0].Typ == ItemType.Initial && res[1].Typ == ItemType.Initial) && res[0].Value == "М" && res[1].Value == "П") && res[2].Typ != ItemType.Initial) 
            {
                res.RemoveAt(0);
                res.RemoveAt(0);
            }
