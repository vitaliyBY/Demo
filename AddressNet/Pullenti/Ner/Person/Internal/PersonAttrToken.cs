...
namespace Pullenti.Ner.Person.Internal
{
    public class PersonAttrToken : Pullenti.Ner.ReferentToken
    {
        ...
        public static PersonAttrToken TryAttach(Pullenti.Ner.Token t, PersonAttrAttachAttrs attrs = PersonAttrAttachAttrs.No)
        {
            ...
            if (res.EndToken.IsValue("ТЕХНИК", null) && (res.EndToken is Pullenti.Ner.TextToken)) 
            {
                if ((res.EndToken as Pullenti.Ner.TextToken).Term != "ТЕХНИК") 
                    return null;
            }
        ...
        static PersonAttrToken CreateAttrPosition(Pullenti.Ner.Core.TerminToken tok, PersonAttrAttachAttrs attrs)
        {
            ...
            for (Pullenti.Ner.Token t = t1.Next; t != null; t = t.Next) 
            {
                    ...
                    if ((t1.Next != null && (t1.WhitespacesAfterCount < 2) && t1.Next.Chars.IsLatinLetter) && !t1.Next.Chars.IsAllLower && Pullenti.Ner.Core.MiscHelper.CheckNumberPrefix(t1.Next) == null) 
                    {
                        for (t = t1.Next; t != null; t = t.Next) 
                        {
                            ...
                            if (t.Kit.BaseLanguage.IsEn) 
                                break;
                            Pullenti.Ner.Token ttt = _analyzeRomanNums(t);
                            if (ttt != null) 
                                t = ttt;
                            t1 = (tname1 = t);
                        }
                ...
                if (npt != null) 
                {
                    if (_existsInDoctionary(npt.EndToken) && (((((npt.Morph.Case.IsGenitive || npt.Morph.Case.IsInstrumental || npt.EndToken.IsValue("НАПРАВЛЕНИЕ", null)) || npt.EndToken.IsValue("ОТДЕЛ", null) || npt.EndToken.IsValue("ОТДЕЛЕНИЕ", null)) || npt.EndToken.IsValue("ДЕПАРТАМЕНТ", null) || npt.EndToken.IsValue("СЛУЖБА", null)) || npt.EndToken.IsValue("ПОДРАЗДЕЛЕНИЕ", null)))) 
                    {
                        test = true;
                        isDepart = true;
                    }
                ...
                else if (t.IsAnd && npt0 != null) 
                {
                    ...
                    if (((npt == null && (t.Next is Pullenti.Ner.TextToken) && t.Next.Chars.IsAllUpper) && t.Next.LengthChar >= 2 && t.Previous != null) && t.Previous.Chars.Equals(t.Next.Chars)) 
                    {
                        t = (t1 = (tname1 = t.Next));
                        npt0 = null;
                        continue;
        ...
        static Pullenti.Ner.Token _analyzeRomanNums(Pullenti.Ner.Token t)
        {
            ...
            else if (tt2.Next != null && tt2.Next.IsCommaAnd) 
            {
                ...
                {
                    tt2 = lat2.EndToken;
                    for (Pullenti.Ner.Token tt = tt2.Next; tt != null; tt = tt.Next) 
                    {
                        if (!tt.IsCommaAnd && !tt.IsHiphen) 
                            break;
                        lat2 = Pullenti.Ner.Core.NumberHelper.TryParseRoman(tt.Next);
                        if (lat2 == null) 
                            break;
                        tt2 = lat2.EndToken;
                        if (tt.IsAnd) 
        ...
        public static Pullenti.Ner.Core.TerminToken TryAttachWord(Pullenti.Ner.Token t, bool ignoreIo = false)
        {
