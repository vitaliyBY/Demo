...
namespace Pullenti.Ner.Person.Internal
{
    class PersonIdToken : Pullenti.Ner.MetaToken
    {
        ...
        static PersonIdToken TryParse(Pullenti.Ner.Token t, PersonIdToken prev)
        {
            ...
            if (tok != null) 
            {
                ...
                if (ty == Typs.Number) 
                {
                    ...
                    for (; tt != null; tt = tt.Next) 
                    {
                        ...
                        {
                            Pullenti.Ner.NumberToken nnn = Pullenti.Ner.Core.NumberHelper.TryParseRoman(tt);
                            if (nnn != null && ((!nnn.IsWhitespaceBefore || tt == tok.EndToken.Next))) 
                            {
                                tmp.Append(Pullenti.Ner.Core.MiscHelper.GetTextValueOfMetaToken(nnn, Pullenti.Ner.Core.GetTextAttr.No));
            ...
            if (prev.Typ == Typs.Keyword || t0 != t) 
            {
                Pullenti.Ner.NumberToken rom = Pullenti.Ner.Core.NumberHelper.TryParseRoman(t);
                if (rom != null) 
                {
                    Pullenti.Ner.Token tt1 = rom.EndToken.Next;
                    if (tt1 != null && tt1.IsHiphen) 
                        tt1 = tt1.Next;
                    if ((tt1 is Pullenti.Ner.TextToken) && (tt1.LengthChar < 4) && tt1.Chars.IsAllUpper) 
                    {
                        PersonIdToken res = new PersonIdToken(t0, tt1) { Typ = Typs.Seria };
                        res.Value = string.Format("{0}-{1}", Pullenti.Ner.Core.MiscHelper.GetTextValueOfMetaToken(rom, Pullenti.Ner.Core.GetTextAttr.No), (tt1 as Pullenti.Ner.TextToken).Term);
                        if (t0 != t) 
                            res.HasPrefix = true;
                        return res;
                    }
        ...
        public static void Initialize()
        {
            ...
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("СВИДЕТЕЛЬСТВО О РОЖДЕНИИ") { Tag = Typs.Keyword };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("СВИДЕТЕЛЬСТВО О СМЕРТИ") { Tag = Typs.Keyword };
            m_Ontology.Add(t);
            t = new Pullenti.Ner.Core.Termin("СПРАВКА О СМЕРТИ") { Tag = Typs.Keyword };
            m_Ontology.Add(t);
