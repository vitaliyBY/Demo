    ...
    public class PersonAnalyzer : Pullenti.Ner.Analyzer
    {
        ...
        public override void Process(Pullenti.Ner.Core.AnalysisKit kit)
        {
            ...
            for (int step = 0; step < steps; step++) 
            {
                ...
                for (Pullenti.Ner.Token t = kit.FirstToken; t != null; t = t.Next) 
                {
                    ...
                    if ((t is Pullenti.Ner.NumberToken) && (t as Pullenti.Ner.NumberToken).Value == "70") 
                    {
                    }
        ...
        internal static Pullenti.Ner.ReferentToken TryAttachPerson(Pullenti.Ner.Token t, bool forExtOntos, int step, bool forAttribute = false)
        {
            ...
            if (ad.OverflowLevel > 2) 
                return null;
            ad.OverflowLevel++;
            Pullenti.Ner.ReferentToken res = _tryAttachPersonInt(t, forExtOntos, step, forAttribute);
            ad.OverflowLevel--;
            return res;
        }
        static Pullenti.Ner.ReferentToken _tryAttachPersonInt(Pullenti.Ner.Token t, bool forExtOntos, int step, bool forAttribute = false)
        {
            ...
            for (; t != null; t = (t == null ? null : t.Next)) 
            {
                ...
                if ((a != null && attrs != null && !t.Chars.IsAllLower) && attrs[0].Typ == Pullenti.Ner.Person.Internal.PersonAttrTerminType.Prefix) 
                {
                    List<Pullenti.Ner.Person.Internal.PersonItemToken> pits = Pullenti.Ner.Person.Internal.PersonItemToken.TryAttachList(t, Pullenti.Ner.Person.Internal.PersonItemToken.ParseAttr.IgnoreAttrs, 10);
                    if (pits != null && pits.Count >= 2 && pits[0].Lastname != null) 
                        a = null;
                }
            ...
            for (int k = 0; k < 2; k++) 
            {
                ...
                if (mi.Case.Equals(Pullenti.Morph.MorphCase.AllCases) && pits.Count > 0) 
                {
                    if (t0.WhitespacesBeforeCount > 4 && pits[pits.Count - 1].WhitespacesAfterCount > 4) 
                        mi.Case = Pullenti.Morph.MorphCase.Nominative;
                }
