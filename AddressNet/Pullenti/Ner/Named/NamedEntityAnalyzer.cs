    ...
    public class NamedEntityAnalyzer : Pullenti.Ner.Analyzer
    {
        ...
        public override Dictionary<string, byte[]> Images
        {
            get
            {
                ...
                res.Add(NamedEntityKind.Award.ToString(), Pullenti.Ner.Core.Internal.ResourceHelper.GetBytes("award.png"));
                res.Add(NamedEntityKind.Art.ToString(), Pullenti.Ner.Core.Internal.ResourceHelper.GetBytes("art.png"));
                return res;
            }
        }
        ...
        static Pullenti.Ner.ReferentToken _tryAttach(List<Pullenti.Ner.Named.Internal.NamedItemToken> toks)
        {
            if (toks == null || toks.Count == 0) 
                return null;
            if ((toks[0].NameValue != null && toks[0].TypeValue == null && toks[0].IsInBracket) && ((toks.Count == 1 || toks[1].TypeValue == null))) 
            {
                NamedEntityReferent parent = null;
                for (Pullenti.Ner.Token tt = toks[0].BeginToken.Previous; tt != null; tt = tt.Previous) 
                {
                    if (tt.IsCommaAnd) 
                        continue;
                    NamedEntityReferent prev = tt.GetReferent() as NamedEntityReferent;
                    if (prev == null) 
                        break;
                ...
                {
                    Pullenti.Ner.Core.AnalyzerDataWithOntology ad = toks[0].Kit.GetAnalyzerDataByAnalyzerName("NAMEDENTITY") as Pullenti.Ner.Core.AnalyzerDataWithOntology;
                    List<Pullenti.Ner.Core.IntOntologyToken> tok = ad.LocalOntology.TryAttach(toks[0].BeginToken.Next, null, false);
                    if (tok != null && tok.Count == 1) 
                        parent = tok[0].Item.Referent as NamedEntityReferent;
                }
                if (parent != null) 
                {
                    NamedEntityReferent ent = new NamedEntityReferent();
                    foreach (Pullenti.Ner.Slot s in parent.Slots) 
                    {
                    ...
                    string norm = Pullenti.Ner.Core.MiscHelper.GetTextValueOfMetaToken(toks[0], Pullenti.Ner.Core.GetTextAttr.FirstNounGroupToNominative);
                    ent.AddSlot(NamedEntityReferent.ATTR_NAME, norm, false, 0);
                    return new Pullenti.Ner.ReferentToken(ent, toks[0].BeginToken, toks[0].EndToken);
                }
            }
