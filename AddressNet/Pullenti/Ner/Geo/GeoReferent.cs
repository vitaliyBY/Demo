...
namespace Pullenti.Ner.Geo
{
    ...
    public class GeoReferent : Pullenti.Ner.Referent
    {
            ...
            if (IsUnion && !IsState) 
            {
                ...
                foreach (Pullenti.Ner.Slot s in Slots) 
                {
                    if (s.TypeName == ATTR_REF && (s.Value is Pullenti.Ner.Referent)) 
                        res.AppendFormat("; {0}", (s.Value as Pullenti.Ner.Referent).ToStringEx(true, lang, 0));
                }
                return res.ToString();
            }
        ...
        string _getName(bool cyr)
        {
            ...
            if (IsCity) 
            {
                string misc = this.GetStringValue(ATTR_MISC);
                if (misc != null && !name.Contains(misc.ToUpper())) 
                    name = string.Format("{0} {1}", misc.ToUpper(), name);
            }
            return name ?? "?";
        }
        ...
        internal void AddTypCity(Pullenti.Morph.MorphLang lang, bool city)
        {
            if (lang.IsEn) 
                this.AddSlot(ATTR_TYPE, (city ? "city" : "locality"), false, 0);
            else if (lang.IsUa) 
                this.AddSlot(ATTR_TYPE, (city ? "місто" : "населений пункт"), false, 0);
            else 
                this.AddSlot(ATTR_TYPE, (city ? "город" : "населенный пункт"), false, 0);
        }
        ...
        {
            if ((((((((((((((v.Contains("поселок") || v.Contains("селение") || v.Contains("городок")) || v.Contains("село") || v.Contains("деревня")) || v.Contains("станица") || v.Contains("пункт")) || v.Contains("станция") || v.Contains("аул")) || v.Contains("улус") || v.Contains("наслег")) || v.Contains("хутор") || v.Contains("слобода")) || v.Contains("местечко") || v.Contains("усадьба")) || v.Contains("починок") || v.Contains("заимка")) || v.Contains("аал") || v.Contains("выселки")) || v.Contains("арбан") || v.Contains("місто")) || v.Contains("селище") || v.Contains("сіло")) || v.Contains("станиця") || v.Contains("станція")) || v.Contains("city") || v.Contains("municipality")) || v.Contains("town")) 
                return true;
            if (v.Contains("порт")) 
                return true;
        ...
        internal void AddOrgReferent(Pullenti.Ner.Referent org)
        {
            ...
            if (!nam) 
                this.AddName(org.ToStringEx(true, Pullenti.Morph.MorphLang.Unknown, 0).ToUpper());
        }
    }
}