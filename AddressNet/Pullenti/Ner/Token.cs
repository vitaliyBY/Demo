...
namespace Pullenti.Ner
{
    ...
    public class Token
    {
        ...
        /// </summary>
        /// <param name="term">слово (проверяется значение TextToken.Term и все морфварианты)</param>
        ...
        /// </summary>
        /// <param name="term">слово (проверяется значение TextToken.Term и все морфварианты)</param>
        /// <param name="nextTerm">слово в следующем токене</param>
        /// <return>да или нет</return>
        public bool IsValue2(string term, string nextTerm)
        {
            if (Next == null) 
                return false;
            if (!this.IsValue(term, null)) 
                return false;
            return Next.IsValue(nextTerm, null);
        }
        /// <summary>
        /// Проверка трёх подряд идущих слова (с учётом морф.вариантов)
        /// </summary>
        /// <param name="term1">первое слово</param>
        /// <param name="term2">слово в след. токене</param>
        /// <param name="term3">слово в третьем токене</param>
        /// <return>да или нет</return>
        public bool IsValue3(string term1, string term2, string term3)
        {
            if (Next == null || Next.Next == null) 
                return false;
            if (!this.IsValue(term1, null)) 
                return false;
            if (!Next.IsValue(term2, null)) 
                return false;
            return Next.Next.IsValue(term3, null);
        }
        /// <summary>
