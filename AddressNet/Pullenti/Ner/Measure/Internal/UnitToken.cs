...
namespace Pullenti.Ner.Measure.Internal
{
    public class UnitToken : Pullenti.Ner.MetaToken
    {
        ...
        public static List<UnitToken> TryParseList(Pullenti.Ner.Token t, Pullenti.Ner.Core.TerminCollection addUnits, bool parseUnknownUnits = false)
        {
            ...
            for (Pullenti.Ner.Token tt = ut.EndToken.Next; tt != null; tt = tt.Next) 
            {
                ...
                if (ut == null) 
                {
                    ...
                    {
                        List<UnitToken> li = TryParseList(tt.Next.Next, addUnits, parseUnknownUnits);
                        if (li != null && li[li.Count - 1].EndToken.Next != null && li[li.Count - 1].EndToken.Next.IsChar(')')) 
                        {
                            foreach (UnitToken v in li) 
                            {
                                v.Pow = -v.Pow;
                                res.Add(v);
                            }
                            li[li.Count - 1].EndToken = li[li.Count - 1].EndToken.Next;
                        }
                    }
                    if (tt.IsCharOf("\\/") || tt.IsHiphen) 
                    {
                        if (!tt.IsWhitespaceAfter && !tt.IsWhitespaceBefore && res.Count == 1) 
                        {
        ...
        public static UnitToken TryParse(Pullenti.Ner.Token t, Pullenti.Ner.Core.TerminCollection addUnits, UnitToken prev, bool parseUnknownUnits = false)
        {
            ...
            if (toks != null) 
            {
                if ((prev != null && tt == t0 && toks.Count == 1) && t.IsWhitespaceBefore) 
                    return null;
                if ((toks[0].LengthChar == 1 && tt.Chars.IsAllUpper && !toks[0].IsWhitespaceAfter) && (toks[0].EndToken.Next is Pullenti.Ner.NumberToken)) 
                    return null;
                ...
                if (toks[0].BeginToken.LengthChar == 1 && toks[0].BeginToken.Chars.IsAllLower) 
                {
                    if (toks[0].BeginToken == toks[0].EndToken || ((toks[0].EndToken.IsChar('.') && toks[0].BeginToken.Next == toks[0].EndToken))) 
                    {
                        ...
                        {
                            if (prev != null && prev.Unit != null && ((prev.Unit.Kind == Pullenti.Ner.Measure.MeasureKind.Length || isNeg))) 
                            {
                                UnitToken res = new UnitToken(t0, toks[0].EndToken) { Unit = UnitsHelper.uSec };
                                res.Pow = pow;
                                if (isNeg) 
                                    res.Pow = -pow;
                                res._correct();
            ...
            if (t.IsCharOf("º°")) 
                t1 = t;
            else if ((t.IsChar('<') && t.Next != null && t.Next.Next != null) && t.Next.Next.IsChar('>') && (((t.Next.IsValue("О", null) || t.Next.IsValue("O", null) || t.Next.IsCharOf("º°")) || (((t.Next is Pullenti.Ner.NumberToken) && (t.Next as Pullenti.Ner.NumberToken).Value == "0"))))) 
                t1 = t.Next.Next;
            if (t1 != null) 
            {
            ...
            if (t.IsChar('%')) 
            {
                ...
                if (tt1 != null && tt1.IsChar('(')) 
                    tt1 = tt1.Next;
                if ((tt1 is Pullenti.Ner.TextToken) && ((tt1.IsValue("ОБ", null) || tt1.IsValue("ОБОРОТ", null)))) 
                {
        ...
        void _checkDoubt()
        {
            ...
            if (Pow != 1) 
                return;
            if ((BeginToken.LengthChar < 3) || BeginToken.IsValue("ARE", null)) 
            {
                IsDoubt = true;
                if ((BeginToken.Chars.IsCapitalUpper || BeginToken.Chars.IsAllUpper || BeginToken.Chars.IsLastLower) || BeginToken.Chars.IsAllLower) 
                {
                    if (BeginToken.LengthChar == 2 && (BeginToken.Previous is Pullenti.Ner.NumberToken) && (BeginToken.WhitespacesBeforeCount < 2)) 
                        IsDoubt = false;
                }
