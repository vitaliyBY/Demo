...
namespace Pullenti.Ner.Core
{
    ...
    public static class MiscHelper
    {
                ...
                if (str != null) 
                {
                    ...
                    if ((end is Pullenti.Ner.MetaToken) && !(end is Pullenti.Ner.NumberToken) && begin.Next == end) 
                    {
                        if (((attrs & GetTextAttr.FirstNounGroupToNominative)) == GetTextAttr.FirstNounGroupToNominative || ((attrs & GetTextAttr.FirstNounGroupToNominativeSingle)) == GetTextAttr.FirstNounGroupToNominativeSingle) 
                        {
                            ...
                            Pullenti.Ner.AnalysisResult ar0 = null;
                            try 
                            {
                                ar0 = Pullenti.Ner.ProcessorService.EmptyProcessor.Process(new Pullenti.Ner.SourceOfAnalysis(string.Format("{0} {1}", str0, str1)), null, null);
        ...
        public static string GetTextMorphVarBySample(string txt, Pullenti.Ner.Token beginSample, bool useMorphSample, bool useRegisterSample)
        {
            ...
            try 
            {
                ar = Pullenti.Ner.ProcessorService.EmptyProcessor.Process(new Pullenti.Ner.SourceOfAnalysis(txt), null, null);
            }
            catch(Exception ex) 
            {
                return txt;
        ...
        public static string GetTextMorphVarByCaseAndNumberEx(string str, Pullenti.Morph.MorphCase cas = null, Pullenti.Morph.MorphNumber num = Pullenti.Morph.MorphNumber.Singular, string numVal = null)
        {
            ...
            try 
            {
                ar = Pullenti.Ner.ProcessorService.EmptyProcessor.Process(new Pullenti.Ner.SourceOfAnalysis(str) { CreateNumberTokens = false }, null, null);
            }
            catch(Exception ex) 
            {
            }
        ...
        /// </summary>
        /// <param name="rt">метатокен, содержаший сущность</param>
        /// <return>более узкий метатокен или этот же, если не удалось уменьшить</return>
        public static Pullenti.Ner.MetaToken GetPureReferentToken(Pullenti.Ner.ReferentToken rt)
        {
            if (rt == null) 
                return null;
            ...
            return new Pullenti.Ner.MetaToken(t0, t1) { Tag = MiscHelper.GetTextValue(t0, t1, GetTextAttr.KeepQuotes | GetTextAttr.KeepRegister) };
        }
    }
}