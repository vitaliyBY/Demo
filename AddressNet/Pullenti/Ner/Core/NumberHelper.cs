    ...
    public static class NumberHelper
    {
        ...
        static Pullenti.Ner.NumberToken _TryParse(Pullenti.Ner.Token token, Pullenti.Ner.NumberToken prevVal = null)
        {
            ...
            if (val != null) 
            {
                ...
                while ((et.Next is Pullenti.Ner.TextToken) && !(et.Previous is Pullenti.Ner.NumberToken) && et.IsWhitespaceBefore) 
                {
                    ...
                    {
                        NumberExToken sub = TryParsePostfixOnly(et.Next.Next);
                        if (sub != null) 
                        {
                            val = val + "000";
                            et = et.Next as Pullenti.Ner.TextToken;
            ...
            for (Pullenti.Ner.TextToken t = tt; t != null; t = t.Next as Pullenti.Ner.TextToken) 
            {
                ...
                if (num == null) 
                    break;
                if (!int.TryParse(num.Termin.Tag.ToString(), out j)) 
                    break;
        ...
        public static Pullenti.Ner.NumberToken TryParseRoman(Pullenti.Ner.Token t)
        {
            ...
            for (; t != null; t = t.Next) 
            {
                ...
                {
                    if ((t is Pullenti.Ner.NumberToken) && (t as Pullenti.Ner.NumberToken).Value == "1" && (t as Pullenti.Ner.NumberToken).Typ == Pullenti.Ner.NumberSpellingType.Digit) 
                        term = "1";
                    else 
                        break;
                }
                ...
                if (ntNext != null) 
                {
                    ...
                    if (t1 is Pullenti.Ner.TextToken) 
                    {
                        ...
                        if (v == "Л" || ((v == "Р" && nt.Morph.Language.IsUa))) 
                        {
                            if (nt.LengthChar < 2) 
                                return null;
                            Pullenti.Ner.NumberToken res = new Pullenti.Ner.NumberToken(t, (t1.Next != null && t1.Next.IsChar('.') ? t1.Next : t1), nt.Value, Pullenti.Ner.NumberSpellingType.Age);
            ...
            if (i > 3) 
            {
                ...
                for (i = ss.Length - 2; i > 2; i--) 
                {
                    ...
                    if (num0.Length > 1 && num0[num0.Length - 1] == '0' && num1.Length == 1) 
                        return new Pullenti.Ner.NumberToken(tt, tt, num0.Substring(0, num0.Length - 1) + num1, Pullenti.Ner.NumberSpellingType.Age) { Morph = tt.Morph };
                }
            }
            return null;
        }
            ...
            if (t1 is Pullenti.Ner.TextToken) 
            {
                ...
                if (v == "ГОД" && t1.Next != null) 
                {
                    ...
                    if (t1.IsValue("ОКТЯБРЬ", null) || t1.IsValue("ПОБЕДА", null)) 
                        return new Pullenti.Ner.NumberToken(t, t1.Previous, nt.Value, Pullenti.Ner.NumberSpellingType.Age);
                }
        ...
        static Pullenti.Ner.Token _tryParseFloat(Pullenti.Ner.NumberToken t, out double d, NumberParseAttr attrs)
        {
            d = 0;
            ...
            for (Pullenti.Ner.Token t1 = (Pullenti.Ner.Token)t; t1 != null; t1 = t1.Next) 
            {
                if (t1.Next == null) 
                    break;
                if (((t1.Next is Pullenti.Ner.NumberToken) && (((t1.WhitespacesAfterCount < 6) || ((t.Previous != null && t.Previous.IsTableControlChar)))) && (t1.Next as Pullenti.Ner.NumberToken).Typ == Pullenti.Ner.NumberSpellingType.Digit) && t1.Next.LengthChar == 3) 
                {
                    if (ns == null) 
                    {
                ...
                if ((t1.Next.IsCharOf(",.") && (t1.Next.Next is Pullenti.Ner.NumberToken) && (t1.Next.Next as Pullenti.Ner.NumberToken).Typ == Pullenti.Ner.NumberSpellingType.Digit) && (t1.WhitespacesAfterCount < 6) && (t1.Next.WhitespacesAfterCount < 2)) 
                {
            ...
            if (sps.Count == 1) 
            {
                if (sps[0] == ' ') 
                    isLastDrob = false;
                else if (((attrs & NumberParseAttr.CommaIsFloatpoint)) != NumberParseAttr.No && sps[0] == ',') 
                    isLastDrob = true;
        ...
        /// </summary>
        /// <param name="t">начальный токен</param>
        /// <param name="attrs">дополнительные атрибуты выделения</param>
        /// <return>числовой метатокен или null</return>
        public static Pullenti.Ner.NumberToken TryParseRealNumber(Pullenti.Ner.Token t, NumberParseAttr attrs = NumberParseAttr.No)
        {
            bool isNot = false;
            ...
            double d;
            Pullenti.Ner.Token tt = _tryParseFloat(t as Pullenti.Ner.NumberToken, out d, attrs);
            if (tt == null) 
            {
                if ((t.Next == null || t.IsWhitespaceAfter || t.Next.Chars.IsLetter) || ((attrs & NumberParseAttr.CanNotBeInteger)) == NumberParseAttr.No) 
                {
                    tt = t;
        ...
        internal static void Initialize()
        {
            ...
            m_Nums.AddString("ТРИЛЬЙОН", "1000000000000", Pullenti.Morph.MorphLang.UA, false);
