...
namespace Pullenti.Ner.Core
{
    ...
    public class Termin
    {
        ...
        /// <summary>
        /// Аббревиатура м.б. мягкой (AcronymSmart = Acronym)
        /// </summary>
        public bool AcronymCanBeSmart;
        /// <summary>
        ...
        public class Abridge
        {
            ...
            public Pullenti.Ner.Core.TerminToken TryAttach(Pullenti.Ner.Token t0)
            {
                ...
                if (Tail.Length > 3) 
                {
                    if (!t1.IsValue(Tail, null)) 
                        return null;
                    if (!t1.Term.StartsWith(Tail)) 
                        return null;
                }
                if (t1.Term != Tail) 
                {
                    if (t1.Term.Length == (Tail.Length + 1)) 
                    {
                        if (t1.Term.StartsWith(Tail) && Pullenti.Morph.LanguageHelper.IsCyrillicVowel(t1.Term[t1.Term.Length - 1])) 
                        {
                        }
                        else 
                            return null;
                    }
                    else if ((t1.Term.Length + 1) == Tail.Length) 
                    {
