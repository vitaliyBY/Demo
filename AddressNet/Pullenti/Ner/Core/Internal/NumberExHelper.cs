...
namespace Pullenti.Ner.Core.Internal
{
    static class NumberExHelper
    {
        public static Pullenti.Ner.Core.NumberExToken TryParseNumberWithPostfix(Pullenti.Ner.Token t)
        {
            ...
            if (nt == null) 
            {
                ...
                for (int i = 4; i < (val.Length - 5); i++) 
                {
                    ...
                    {
                        Pullenti.Ner.Core.NumberExToken re = new Pullenti.Ner.Core.NumberExToken(t, t, li[0].Tag.ToString(), Pullenti.Ner.NumberSpellingType.Words, (Pullenti.Ner.Core.NumberExType)lii[0].Tag) { Morph = t.Morph };
                        _correctExtTypes(re);
                        return re;
                    }
                    break;
            ...
            {
                double d;
                Pullenti.Ner.NumberToken tt11 = Pullenti.Ner.Core.NumberHelper.TryParseRealNumber(nt, Pullenti.Ner.Core.NumberParseAttr.CanNotBeInteger);
                if (tt11 != null) 
                {
            ...
            if (toks != null) 
            {
                ...
                if (toks.Termin.CanonicText == "LTL") 
                    return null;
                if (toks.Termin.CanonicText == "BGN" && (t1 is Pullenti.Ner.TextToken)) 
                {
                    if ((t1 as Pullenti.Ner.TextToken).Term == "ЛЬВА" || (t1 as Pullenti.Ner.TextToken).Term == "ЛЕВ") 
                        return null;
                }
        ...
        internal static void Initialize()
        {
            ...
            t.AddAbridge("Y.E.");
            m_Postfixes.Add(t);
            bool bb = Pullenti.Ner.Core.Termin.AssignAllTextsAsNormal;
            ...
            for (int k = 0; k < 3; k++) 
            {
                ...
                foreach (string line0 in str.Split('\n')) 
                {
                    ...
                    if (string.IsNullOrEmpty(parts[1]) || string.IsNullOrEmpty(parts[2])) 
                        continue;
                    t = new Pullenti.Ner.Core.Termin(parts[1], lang);
                    ...
                    foreach (string p in parts[0].Split(',')) 
                    {
                        if (p != parts[1]) 
                        {
                            Pullenti.Ner.Core.Termin t0 = new Pullenti.Ner.Core.Termin(p, lang);
                            t.AddVariantTerm(t0);
                        }
                    }
                    if (parts[1] == "РУБЛЬ") 
            ...
            Pullenti.Ner.Core.Termin.AssignAllTextsAsNormal = bb;
        }
